...
def apply_rotary_emb(
    ...
    # 安全地重塑形状以分离实部和虚部
    # 检查最后一个维度是否可以被2整除，避免重塑错误
    if xq_float.shape[-1] % 2 != 0:
        # 如果不能被2整除，可以选择填充或截断，但这里我们假设模型设计保证了维度的正确性
        # 为了安全起见，我们使用整除来计算重塑的维度
        new_dim = xq_float.shape[-1] // 2
        xq_r, xq_i = xq_float.reshape(xq_float.shape[:-1] + (new_dim, 2)).unbind(-1)
    else:
        # 正常情况下的重塑
        xq_r, xq_i = xq_float.reshape(xq_float.shape[:-1] + (-1, 2)).unbind(-1)
    
    # 对键张量执行相同的安全重塑
    if xk_float.shape[-1] % 2 != 0:
        new_dim = xk_float.shape[-1] // 2
        xk_r, xk_i = xk_float.reshape(xk_float.shape[:-1] + (new_dim, 2)).unbind(-1)
    else:
        xk_r, xk_i = xk_float.reshape(xk_float.shape[:-1] + (-1, 2)).unbind(-1)
...
class Transformer(nn.Module):
    ...
    def forward(self, tokens:torch.Tensor, targets: Optional[torch.Tensor]=None) ->torch.Tensor:
        ...
        h = self.tok_embedding(tokens) #[bs, seq_len, dim]
        h = self.dropout(h)
